...
public class GameController implements Initializable {
   ...
   */
  final Color squareFillColor = Color.TRANSPARENT;

  /**
   * The border color of a single square
   */
  final Color squareForeColor = Color.DARKBLUE;

  /**
   * Represents an empty space on the board
   */
  final String emptyMark = "-";

  /**
   * Represents a hit on a ship on the board
   */
  final String hitMark = "x";

  /**
   * Represents a miss on a ship on the board
   */
  final String missMark = "o";
   ...
   */
  private ObservableList<Pair<StringProperty, StringProperty>> scoreData;

  /**
   * List of pairs (name, score) to hold the top time scores sorted from lowest to
   * highest time
   */
  SortedList<Pair<StringProperty, StringProperty>> sortedData;
  ...
  void initScoreBoard() {
    ...
    scoreData.add(new Pair<StringProperty, StringProperty>(new SimpleStringProperty("Sandeep"),
        new SimpleStringProperty("00:66:33")));
    scoreData.add(new Pair<StringProperty, StringProperty>(new SimpleStringProperty("Harsh"),
        new SimpleStringProperty("00:30:10")));
    scoreData.add(new Pair<StringProperty, StringProperty>(new SimpleStringProperty("Urbashi"),
        new SimpleStringProperty("00:10:10")));
    scoreData.add(new Pair<StringProperty, StringProperty>(new SimpleStringProperty("Emmanuel"),
        new SimpleStringProperty("00:20:10")));
    scoreData.add(new Pair<StringProperty, StringProperty>(new SimpleStringProperty("Uchechukwu"),
    ...
    sortedData = new SortedList<>(scoreData);
    sortedData.comparatorProperty().bind(tviewScores.comparatorProperty());

    tviewScores.setItems(sortedData);
   ...
   *          4=destroyer)
   * @return true if valid position for ship placement, false otherwise
   */
  boolean positionsValid(Player p, int row, int col, int size, boolean vertical, String type) {
    System.out.print("positionsValid(" + row + "," + col + ");size=" + size + ":"+" type= "+type);
    if (vertical) {
      ...
      if (row - 1 >= 0) {
        ...
        if (col - 1 >= 0) {
          if (col + 1 <= 9) // checking both sides
          {
            ...
            if ((!p.getBoard()[y][col - 1].equals(emptyMark) && !p.getBoard()[y][col - 1].equals(type))
                || (!p.getBoard()[y][col].equals(emptyMark) && !p.getBoard()[y][col].equals(type))
                || (!p.getBoard()[y][col + 1].equals(emptyMark) && !p.getBoard()[y][col + 1].equals(type))) {
              return false;
            }
          } else // checking left
          {
        ...
        {
          if ((!p.getBoard()[y][col].equals(emptyMark) && !p.getBoard()[y][col].equals(type))
              || (!p.getBoard()[y][col + 1].equals(emptyMark) && !p.getBoard()[y][col + 1].equals(type))) {
            return false;
          }
        }
      ...
      if (col - 1 >= 0) {
        ...
        if (row - 1 >= 0) {
          if (row + 1 <= 9) // checking both sides
          {
            ...
            if ((!p.getBoard()[row - 1][x].equals(emptyMark) && !p.getBoard()[row - 1][x].equals(type))
                || (!p.getBoard()[row][x].equals(emptyMark) && !p.getBoard()[row][x].equals(type))
                || (!p.getBoard()[row + 1][x].equals(emptyMark) && !p.getBoard()[row + 1][x].equals(type))) {
              return false;
            }
          } else // checking top
          {
        ...
        {
          if ((!p.getBoard()[row][x].equals(emptyMark) && !p.getBoard()[row][x].equals(type))
              || (!p.getBoard()[row + 1][x].equals(emptyMark) && !p.getBoard()[row + 1][x].equals(type))) {
            return false;
          }
        }
   ...
   *          4=destroyer)
   */
  void positionsSet(Player p, int row, int col, int size, boolean vertical, String type) {
    if (vertical) {
      for (int y = row; y < size + row; y++) {
  ...
  void placeShips(Player p) {
    // init board to empty marks
    for (int i = 0; i < 10; i++) {
      for (int j = 0; j < 10; j++) {
        p.getBoard()[i][j] = emptyMark;
      }
    }
    ...
    for (int b = 0; b < 5; b++) {
      ...
      while (true) {
        ...
        Ship s = new Ship(row, col, sizes[b], vertical);

        if (positionsValid(p, row, col, sizes[b], vertical, String.valueOf(b))) {
          p.getShips()[b] = s;
          positionsSet(p, row, col, sizes[b], vertical, String.valueOf(b));
          break;
        }
      }
    }
  ...
  void onMouseEntered(Player player, GridPane gridPlayer, MouseEvent mouseEvent) {
    ...
    if (dragged) {
      ...
      if (positionsValid(player, row, col, player.getShips()[iId].getSize(), player.getShips()[iId].isVertical(),
        ...
        // delete old pos
        positionsSet(player, oldS.getRow(), oldS.getCol(), oldS.getSize(), oldS.isVertical(), emptyMark);
        Utils.printBoard(player);

        // set new pos
        positionsSet(player, row, col, sizes[iId], oldS.isVertical(), sId);
        Utils.printBoard(player);
        Ship newS = new Ship(row, col, sizes[iId], oldS.isVertical());
    ...
    if (!player.getBoard()[row][col].equals(emptyMark)) {
  ...
  void onMouseDragged(Player player, GridPane gridPlayer, MouseEvent mouseEvent) {
    ...
    if (button == MouseButton.PRIMARY) {
      ...
      if (!player.getBoard()[row][col].equals(emptyMark) && !dragged) {
  ...
  void onMouseClickedRight(Player player, GridPane gridPlayer, MouseEvent mouseEvent) {
    ...
    if (player.getBoard()[row][col] != emptyMark) {
      ...
      loopExit: for (int rr = rStart; rr <= rEnd; rr++) {
        for (int cc = cStart; cc <= cEnd; cc++) {
          if (positionsValid(player, rr, cc, oldS.getSize(), !oldS.isVertical(), sId)) {
            ...
            // set new pos
            positionsSet(player, rr, cc, sizes[iId], !oldS.isVertical(), sId);
            Utils.printBoard(player);
            Ship newS = new Ship(rr, cc, sizes[iId], !oldS.isVertical());
            player.getShips()[iId] = newS;
            updateBoardDisplay(player, gridPlayer, true, 0);
            break loopExit;
          }
  ...
  void setGridPlayer1HumanListeners() {
    for (Node node : gridPlayer1.getChildren()) {
      ...
      rect.setStroke(squareForeColor);
      if (!player1.getBoard()[y][x].equals(emptyMark)) {
            ...
            if (gameMode) {
              ...
              if (!turnPlayer1) {
                if (chkSalvoMode.isSelected()) {
                  if (player1.getBoard()[row][col] != hitMark && player1.getBoard()[row][col] != missMark) {
  ...
  void setGridPlayer2HumanListeners() {
    for (Node node : gridPlayer2.getChildren()) {
      ...
      rect.setStroke(squareForeColor);
      if (!player2.getBoard()[y][x].equals(emptyMark)) {
        ...
        public void handle(MouseEvent mouseEvent) {
          ...
          if (button == MouseButton.SECONDARY) {
            ...
            if (gameMode) {
              ...
              if (turnPlayer1) {
                if (chkSalvoMode.isSelected()) {
                  if (player2.getBoard()[row][col] != hitMark && player2.getBoard()[row][col] != missMark) {
  ...
  boolean doRandom() {
    ...
    while (true) { // stay here until hit or miss
      ...
      if (player1.getBoard()[rr][cc] != hitMark && player1.getBoard()[rr][cc] != missMark) {
        if (player1.getBoard()[rr][cc] == emptyMark) {
          // miss
          player1.getBoard()[rr][cc] = missMark;
          break;
        } else {
          // hit
          int index = Integer.valueOf(player1.getBoard()[rr][cc]);
          player1.getBoard()[rr][cc] = hitMark;
        ...
        for (Ship s : player1.getShips()) {
          if (s.isVertical()) {
            ...
            for (int y = s.getRow(); y < s.getRow() + s.getSize(); y++) {
              int rr = y;
              if (player1.getBoard()[rr][cc] != hitMark && player1.getBoard()[rr][cc] != missMark
                  && player1.getBoard()[rr][cc] != emptyMark) {
                // hit
                int index = Integer.valueOf(player1.getBoard()[rr][cc]);
                player1.getBoard()[rr][cc] = hitMark;
                player1.getShips()[index].setHits(player1.getShips()[index].getHits() + 1);
  ...
  void playerHumanSinglePlay(int row, int col, Player player1, GridPane gridPlayer1, Player player2,
    ...
    if (player2.getBoard()[row][col] == emptyMark) {
      // miss
      player2.getBoard()[row][col] = missMark;
    } else {
      // hit
      int index = Integer.valueOf(player2.getBoard()[row][col]);
      player2.getBoard()[row][col] = hitMark;
      ...
      if (player2.getShips()[index].getHits() == player2.getShips()[index].getSize()) {
        ...
        if (Utils.isGameOver(player2)) {
          ...
          mediaPlayerClick = new MediaPlayer(soundExplosion);
          mediaPlayerClick.play();
          Utils.showInfo("Ship down: " + player2.getName(), true);
        }
      }
    }
    if (chkSalvoMode.isSelected()) {
   ...
   * @author Harsh
   * @param event Event triggered when a selection is made
   */
  @FXML
  ...
  /**
   * Changes the mouse cursor and enables the edit mode when hovering over Player1's name
   */
  @FXML
  void onPlayer1NameMouseClicked() {
    System.out.println("onPlayer1NameClicked()");
  ...
  void onPlayer1NameMouseEntered() {
    System.out.println("onPlayer1NameEntered()");
  ...
  void onPlayer1NameMouseExited() {
    System.out.println("onPlayer1NameMouseExited()");
  ...
  void onPlayer2NameMouseEntered() {
    System.out.println("onPlayer2NameEntered()");
  ...
  void onPlayer2NameMouseExited() {
    System.out.println("onPlayer2NameMouseExited()");
